...
\title[Mechanism of the Structural Backfire Effect]{\dgreen{Mechanism of the Structural Backfire Effect: \\How Hub Opinion Shifts Fragment Heavy-Tailed Social Networks}}
...
through structural fragmentation of the network, with little opinion-level repulsion
...
they do not address the consequences of hub users' \emph{own} opinion shifts---whether
...
attacking actors rather than on the network position of the targeted hub:
...
conditions under which the opinion shift of a \emph{human} hub user---endogenous or
exogenous---triggers irreversible polarization.
...
examine the mechanism, boundary conditions, and robustness of this phenomenon---
...
\item[{\bf Social Reinforcement.}]
 Agents increase or decrease
  ...
  \leq \epsilon_{\min}\}$ (comfortable posts, as defined above). The posting
...
\dgreen{The update of the bounded confidence threshold reflects the asymmetric
  ...
  conflicting content erodes openness while comfortable content does not
  symmetrically restore it, so $\BoundedConfidence_i$ evolves monotonically
  ...
  $0 < \delta_{\BoundedConfidence} < 1$ denotes the multiplicative decay rate of confidence in the surrounding environment.
  }
\end{description}
...
parameter settings ($\ThofOpPreval = 0.5$, $\Stubbornness = 0.8$, Holme--Kim network with $p_t = 0.3$).
...
the opposing camp unfollows it nearly four times as often as the targeted camp
...
(the comfort rate cRate, i.e.\ the aligned fraction of the feed defined via $\epsilon_{\min}$ in the Social Reinforcement rule)
...
\Dgreen
To isolate the role of the manipulated user's hub status
--- rather than that of the heavy-tailed topology
...
\section{Conclusion}
\Dgreen
This paper establishes the structural backfire effect---the
...
identifies the structural mechanism that produces it. Building on a
...
was essentially absent in networks lacking a heavy tail; 
...
agents who never relied on it---into homogeneous feeds.
\par
...
equilibrium can be anchored to established stylized facts, no comparable empirical
regularities have yet been documented for the post-backfire regime---the very
...
opinion is abstracted as a single one-dimensional issue and

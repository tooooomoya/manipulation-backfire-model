...
# 80 raw seeds (was 100). Validity ~83% mean (min 54%) => ~50 valid selected in
# most conditions; worst case ~43 (minor power cut). Cuts sim time ~20%. Applies
# to conditions run from the next restart onward (1-13 already done with 100).
TARGET_SEEDS="$(seq 0 79 | tr '\n' ' ')"
MAX_PARALLEL=18   # 20-core box; 18 java x 2g heap ~36g < RAM. (was 10)
JAVA_HEAP="2g"
LOGDIR="$PROJ/logs"
